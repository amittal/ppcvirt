...
cross-ISA portability\cite{bansal:osdi08, qemu}, hardware-based
performance acceleration\cite{transmeta_crusoe}, runtime compiler
optimizations\cite{dynamo}, security and correctness\cite{dynamorio},
testing and debugging\cite{valgrind}. Binary translation was
first used for efficient virtualization of the x86 architecture by
VMware\cite{agesen:comparison}, and our work
is perhaps closest to their approach. The difference is in the design of the
binary translator, as discussed previously (Section~\ref{sec:comparison_with_vmware}).
