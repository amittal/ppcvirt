...
comparison. In a way, the number of exits occurring in {\tt KVM-PV} is the best
achievable by any host-side binary translation solution.
...
reduces the access violations. As we will see later, Optimization~3 also helps
...
Table~\ref{tab:adaptive_tlb_resizing_exits}). The microbenchmarks
are also short-lived, but they have a much shorter memory footprint (as compared
to {\tt find}) and hence do not show a large increase in TLB misses.
\begin{table*}
\centering
