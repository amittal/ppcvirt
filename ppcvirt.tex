...
\maketitle
\begin{abstract}
  Power Architecture\textsuperscript{\textregistered} platforms are popular and widespread on embedded systems, and such
  platforms are
  increasingly
  being used to run virtual machines\cite{embedded_virtualization, KVM_on_embedded_Power}. While the Power
  Architecture platforms meet the
...
guest and host sides for Power Architecture\cite{pvpower} platforms. The paravirtual extension in the guest
...
are fixed length and word aligned on Power Architecture platforms, this ensures that
...
exacerbated on embedded Power Architecture platforms, where OS typically uses
...
on embedded
Power Architecture platforms uses huge pages for kernel code/data to minimize
...
{\tt execute-only}. This is possible on embedded Power Architecture processors due to the
