...
An operating system typically tries to use large TLB entries for kernel address space to
...
(0xc0000000,0xc0800000);(0xc0800000,0xc0c00000);(0xc0c00000,0xc1000000);
(0xc1000000,0xc2000000);(0xc2000000,0xc4000000);(0xc4000000,0xc8000000);
...
The page size of smallest TLB entry, containing the patch-site, needs to be chosen
wisely.
A large page size can cause more page faults in future.
A small page size can result in a large number of TLB entries, which can potentially
result in TLB thrashing. This three-way trade-off between the number of
privileged exits, the number of tracing page faults, and TLB thrashing is
delicate and requires careful performance tuning.

We use the following policy. On a page fault, the size of the page
containing the patch-site is halved, until a minimum of 256KB-sized pages.
The lower bound on page size (256K) limits the maximum number of TLB entries.
TLB entries with identical privileges are also opportunistically merged into
larger TLB entries. For example, merging happens when all patch-sites are
removed from a page, and it has identical {\tt rwx} privileges as its neighbour.
